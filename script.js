...
function makePageForShows(showList1) {
  let showList = [];
  rootElem.innerHTML = "";
  ...
  for (let i = 0; i < showList.length; i++) {
    ...
    if (showList[i].summary.length < char_limit) {
      showSummary.innerHTML = showList[i].summary;
    } else {
      var more = true;
...
function sortshows(arrayOfObjects) {
  let arrTexts = [];
  let sortedArrayOfObjects = [];

  for (let i = 0; i < arrayOfObjects.length; i++) {
    arrTexts[i] = arrayOfObjects[i].name;
  }
